...
export async function upsertEpargneMensuelleAction(
  ...
  // ── Mise à jour du compte principal (epargneActuelle) ────────────────────
  // Seule la portion "standard" alimente le compte principal.
  // Les autres portions vont directement dans leur compte respectif (déjà traité ci-dessus).
  const standardNew = repartitionRaw["standard"] ?? (hasRepartition ? 0 : montant);
  const standardOld = (oldRepartition["standard"] ?? (Object.keys(oldRepartition).length === 0 ? oldMontant : 0));
  const delta = standardNew - standardOld;
...
export async function deleteEpargneMensuelleAction(id: string) {
  ...
  // ── Identifier les comptes référencés dans la répartition ───────────────
  // On cherche TOUS les comptes de l'utilisateur référencés (actifs ou non),
  // car un compte peut avoir été désactivé entre la saisie et la suppression.
  const compteIds = Object.keys(repartition).filter(
    (k) => k !== "standard" && k !== "imprevus" && !k.startsWith("imprevu_")
  );

  const comptesSpeciaux =
    compteIds.length > 0
      ? await prisma.compte.findMany({
          where: { id: { in: compteIds }, userId },
        })
      : [];

  // Décrémenter les soldes des comptes spéciaux
  const updates: Promise<unknown>[] = [];

  for (const compte of comptesSpeciaux) {
    const montantCompte = repartition[compte.id];
    if (montantCompte && montantCompte > 0) {
      updates.push(
        prisma.compte.update({
          where: { id: compte.id },
  ...
  // Décrémenter uniquement la portion standard du compte principal
  const standardPortion = repartition["standard"] ??
    (Object.keys(repartition).filter(k => k !== "imprevus" && !k.startsWith("imprevu_")).length === 0
      ? entry.montant
      : 0);

  await prisma.$transaction([
    prisma.epargneMensuelle.delete({ where: { id } }),
    ...(standardPortion > 0 ? [prisma.user.update({
      where: { id: userId },
      data: { epargneActuelle: { decrement: standardPortion } },
    })] : []),
  ]);
